...
- *RPG complété*: add missing crop field data that was not officially reported in the Common Agricultural Policy (CAP) so absent from the RPG dataset. It uses a combination of datasets from cadastre, IGN BD TOPO and OSO. Data is publicly available since 2018-2023 and could be retrieve for the year 2016-2017. It is spread in multiple files per regions or department <https://entrepot.recherche.data.gouv.fr/dataverse/rpg_complete_2022>. The main issue is that the definition of parcelle is different from cadastre, so if may bring biases in the vectorial calculation (based on the field definition). It is heavy to process and bring little information on land cover. Can be  
...
```{r}
#| label: fig-17
#| fig-cap: "Field area in ha per project"
...
- Some fields are defined as ```r ref$nom[ref$original_code == "BOR"]``` which are not field but borders (as in @fig-5). Should we remove non crop fields from RPG before running the calculations?


### Hedgerows length

Using the field as defined by RPG, we calculated the length of hedgerows from BD Haies that intersect the field (+ a small buffer).
...
Using the coordinated of the sampling sites, we calculated the average area of all crop fields within a buffer (500m, 1000m, and 1500m).
...
We see that some observations don't have crop field within 500m (N=13). These observations (listed in @tbl-7) would need to be checked and ensure that they are close to an agricultural field.  
