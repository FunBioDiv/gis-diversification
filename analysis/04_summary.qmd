...
There is a strong relation between area and perimeter (@fig-3). In median, field size is ```r round(median(indv$Area_ha, na.rm=TRUE),1)``` ha and field perimeter is ```r round(median(indv$Perim_m, na.rm=TRUE),0)```.  
...
```{r}
#| label: tbl-5
#| tbl-cap: "Summary statistics of the hedgerows length (in m) within different buffer size"
buff_hedge <- cbind(
...
**To be discussed:**
- which buffer size to calculate the hedgerows lengths? Is 10m large enough, or more? 
- should we consider the position of the field sampling ?
...
We see that some observations don't have crop field within 500m (N=13). To be checked (@tbl-7) whether those observations were really made close to an agricultural field.
...
**To be discussed**  
- RPG complété: need more data processing, and might not add much in term of crop cover (but in some wineyards)

- As seen in @fig-2, some coordinates were taken at the edge or on the boundary of the field, so it is not possible to clearly identify the field. In such case, should we consider the closest field within a distance threshold (e.g. 10m)?

- add indicators about position on the field? were the samples taken from the edges or close to center?
...
List of the most commun land cover at year N:  
```{r}
...
paste_name <- function(x) {
  return(paste0(names(x), " (", round(x * 100, 1), "%)"))
}
...
#fmt:skip
mean_500_gr <- tapply(mean_500, ref$grp_label[match(names(mean_500), ref$new_code)], sum)

mean_1000 <- apply(indr[, m1000], 2, mean, na.rm = TRUE)
names(mean_1000) <- gsub("frac1000_", "", names(mean_1000))
#fmt:skip
mean_1000_gr <- tapply(mean_1000, ref$grp_label[match(names(mean_1000), ref$new_code)], sum)

mean_1500 <- apply(indr[, m1500], 2, mean, na.rm = TRUE)
names(mean_1500) <- gsub("frac1500_", "", names(mean_1500))
#fmt:skip
mean_1500_gr <- tapply(mean_1500, ref$grp_label[match(names(mean_1500), ref$new_code)], sum)
...
top1 <- c(
  paste_name(sort(mean_500_gr)[length(mean_500_gr)]),
  paste_name(sort(mean_1000_gr)[length(mean_1000_gr)]),
  paste_name(sort(mean_1500_gr)[length(mean_1500_gr)])
)
top2 <- c(
  paste_name(sort(mean_500_gr)[length(mean_500_gr) - 1]),
  paste_name(sort(mean_1000_gr)[length(mean_1000_gr) - 1]),
  paste_name(sort(mean_1500_gr)[length(mean_1500_gr) - 1])
)
top3 <- c(
  paste_name(sort(mean_500_gr)[length(mean_500_gr) - 2]),
  paste_name(sort(mean_1000_gr)[length(mean_1000_gr) - 2]),
  paste_name(sort(mean_1500_gr)[length(mean_1500_gr) - 2])
)


info <- rbind(n_classes, top1, top2, top3)
...
keepC <- unique(c(
  names(mean_500_gr)[mean_500_gr > 0.02],
  names(mean_1000_gr)[mean_1000_gr > 0.02],
  names(mean_1500_gr)[mean_1500_gr > 0.02]
))
