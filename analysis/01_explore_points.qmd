...
Fields in the `RPG sequence de culture` can be sub-divided depending in the time period 2015-2023. So we have to re-calculate the size of the entire field of the year when the study was carried out.   
...
```{r}
#| label: fig-5
#| fig-cap: "Correlations among field areas in the time series. The subarea is the area from sub-fields provided directly in the RPG sequence de culture. The areas are calculated from all the fields with the same ID in a given year."
...
**TO BE DISCUSSED**:    
- the correlation in field areas in the successive years are (relatively) low compared to my expectations (field size shouldn't change so much in 10-year period, and more closely related years should be more correlated). Are the results shown in @fig-5 normal or does it indicate possible errors in merging subfields?
...
field_buff500 <- buffer(rpg_simple, 500)
field_buff1000 <- buffer(rpg_simple, 1000)
field_buff1500 <- buffer(rpg_simple, 1500)

plot(field_buff1500[1])
plot(field_buff1000[1], add=TRUE, border="blue")
plot(field_buff500[1], add=TRUE, border="red")
plot(field_buff10[1], add=TRUE, border = "darkgreen")
plot(rpg_simple[1], add=TRUE, col="grey")
legend("topleft", legend=c(1500, 1000, 500, 10), col=c("black", "blue", "red", "darkgreen"), lwd=1, title="Buffer size")
```


```{r}
#| label: fig-13
#| fig-cap: "Relationships among hedge density calculated with different buffer sizes (1500m, 1000m, 500m, 10m, and no buffer)."


haie_1500 <- intersect(haie, field_buff1500)
length_1500 <- tapply(perim(haie_1500), haie_1500$ID, sum)

haie_1000 <- intersect(haie, field_buff1000)
length_1000 <- tapply(perim(haie_1000), haie_1000$ID, sum)

haie_500 <- intersect(haie, field_buff500)
length_500 <- tapply(perim(haie_500), haie_500$ID, sum)
...
density_buffer <- data.frame(
  b1500 = length_1500[rpg_simple$ID]/expanse(field_buff1500)*10,
  b1000 = length_1000[rpg_simple$ID]/expanse(field_buff1000)*10,
  b500 = length_500[rpg_simple$ID]/expanse(field_buff500)*10,
...
The edge densities calculated with a large buffer (>500m) are highly correlated, especially the buffers 1000m and 1500m. The local hedge density might be better estimated with a buffer of 10m around the field (it will solve the issue seen in @fig-11). 
